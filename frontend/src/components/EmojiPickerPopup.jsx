...
const EmojiPickerPopup = ({ icon, onSelect }) => {
  ...
  return (
    <div className="flex flex-col md:flow-row items-start gap-5 mb-6">
      <div className="flex items-center gap-4 cursor-pointer" onClick={() => setIsOpen(true)}>
        <div className="w-12 h-12 flex items-center justify-center text-2xl bg-purple-50 text-primary rounded-lg">
          {icon ? <img src={icon} alt="Icon" className="w-12 h-1/2"></img> : <LuImage />}
        </div>
        <p className="">{icon ? "Change Icon" : "Pick Color"}</p>
      </div>
      {isOpen && (
